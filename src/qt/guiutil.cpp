...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BeetleCoin address (e.g. %1)").arg("CV7AeX9sYDiL2GSX9PhQzKFTgzmPUQnxX3"));
#endif
...
{
    // return if URI is not valid or is no BeetleCoin: URI
...
{
    // Convert beetlecoin:// to beetlecoin:
    //
    //    Cannot handle this later, because beetlecoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open beetlecoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BeetleCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BeetleCoin.lnk
...
{
    return GetAutostartDir() / "beetlecoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a beetlecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BeetleCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the beetlecoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add beetlecoin app to startup item list
