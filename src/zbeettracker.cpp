...
CzBEETTracker::CzBEETTracker(std::string strWalletFile)
{
...
CzBEETTracker::~CzBEETTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBEETTracker::Init()
{
...
bool CzBEETTracker::Archive(CMintMeta& meta)
{
...
bool CzBEETTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBEETTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBEETTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBEETTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBEETTracker::GetSerialHashes()
{
...
CAmount CzBEETTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBEETTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBEETTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBEETTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBEETTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBEETTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBEETTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBEETTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBEETTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBEETTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBEETTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBEETTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBEETTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBEETTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBEETTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBEETTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBEETTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbeet from DB\n", __func__, listDeterministicDB.size());
    }
