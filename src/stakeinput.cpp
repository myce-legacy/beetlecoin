...
CZBeetStake::CZBeetStake(const libzerocoin::CoinSpend& spend)
{
...
int CZBeetStake::GetChecksumHeightFromMint()
{
...
int CZBeetStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZBeetStake::GetChecksum()
{
    return nChecksum;
}

// The zBEET block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZBeetStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZBeetStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZBeetStake::GetModifier(uint64_t& nStakeModifier)
{
...
bool CZBeetStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZBeetStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zBEET that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBEETOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBEET output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBEET", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBEETOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBEET output", __func__);
...
bool CZBeetStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZBeetStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzBEETTracker* zbeetTracker = pwallet->zbeetTracker.get();
    CMintMeta meta;
    if (!zbeetTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbeetTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
